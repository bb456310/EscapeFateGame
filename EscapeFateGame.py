...
class Character:
    ...
    def attack(self, enemy_char):
        if self.has_sword == True and enemy_char.has_armor == False:
            print("{my_name} has attacked {enemy_name} for {damage} damage!".format(my_name=self.name, enemy_name=enemy_char.name, damage=round(self.level * 2)))
        ...
        if self.has_sword == True and enemy_char.has_armor == True:
            print("{my_name} has attacked {enemy_name} for {damage} damage!".format(my_name=self.name, enemy_name=enemy_char.name, damage=round(self.level)))
        ...
        if self.has_sword == False and enemy_char.has_armor == True:
            print("{my_name} has attacked {enemy_name} for {damage} damage!".format(my_name=self.name, enemy_name=enemy_char.name, damage=round(self.level * .5)))
    ...
    def level_up(self):
        self.level += 1
        print("{name} has levelled up!".format(name=self.name))
...
###need to change use potion to use item from inventory <<<< only if more items are added
    ...
    def run(self):
        print("{name} bolts from the battle.".format(name=self.name))
    ...
    def defend(self, enemy_char):
        ...
        print("{name} braces himself for the upcoming enemy blow!".format(name=self.name))

    def loot_chest(self):
        loot = random.choices(item_list, weights=(3,5,1,3), k=1)
        if loot == ["Potion"]:
            self.num_potions += 1
            print("Inside is a potion!")
        if loot == ["Longsword"]:
            self.has_sword = True
            self.inventory += "Longsword"
            print("Inside is a Longsword!")
        if loot == ["Shield"]:
            self.has_armor = True
            self.inventory += "Shield"
            print("Inside is a Shield!")
        if loot == ["Magic Mushroom"]:
            self.inventory += "Magic Mushroom"
            print("Inside is a Mysterious Magic Mushroom!")
            ...
            if shroom_choice == "yes":
                print("{name} suddenly feels much stronger!".format(name=self.name))
                self.level_up
    ...
    def spring_trap(self):
        ...
        if trap_choice == "False Floor":
            print("{name} stumbles across a False Floor, dropping down into hidden spikes below!".format(name=self.name))
        if trap_choice == "Trip Wire":
            print("{name} feels his foot catch on a Trip Wire. Suddenly his world is red as a fiery explosion engulfs his body!".format(name=self.name))
        if trap_choice == "Poison Darts":
            print("{name} steps cautiously across the floor - but not cautiously enough! A pressure plate triggers a barrage of Poison Darts!".format(name=self.name))
...
class Enemy:
    ...
    def __repr__(self):
        return_phrase = "The {name} stands in the way of the exit and must be vanquished!".format(name=self.name)
        if self.has_armor == True:
            return_phrase += "{name} is donned in armor. Strike wisely.".format(name=self.name)
        if self.has_sword == True:
            return_phrase += "{name} wields a longsword. Be ware!".format(name=self.name)
    ...
    def attack(self, user_char):
        ...
        if self.has_sword == True and user_char.has_armor == False:
            ...
            print("{enemy_name} has attacked {user_name} for {damage} damage!".format(enemy_name=self.name, user_name=user_char.name, damage=round(self.strength * 2)))
        ...
        if self.has_sword == True and user_char.has_armor == True:
            ...
            print("{enemy_name} has attacked {user_name} for {damage} damage!".format(enemy_name=self.name, user_name=user_char.name, damage=round(self.strength)))
        ...
        if self.has_sword == False and user_char.has_armor == True:
            ...
            print("{enemy_name} has attacked {user_name} for {damage} damage!".format(enemy_name=self.name, user_name=user_char.name, damage=round(self.strength * .5)))
...
god = Character("God", 1000, True, True, 100)
...
giant = Enemy("Giant", 15, False, True, 6)
...
while begin != "yes":
    ...
    begin = begin.lower()
if begin == "yes":
    print("{name} looks around the cell.".format(name=character_select.name))
...
##############################################################################################################
### first branch    
room_choice = input("There's nothing more in this room. It's time to move on.\nThere are seemingly two exits: through the broken cell door, or through a large crevice in the back of the wall. \nWhich do you take: Door or Crevice?")
room_choice = room_choice.lower()
if room_choice == "door":
    print("{name} makes his way over to the cell door. The broken door appears to be stuck.".format(name=character_select.name))
    break_choice = input("Do you attempt to break through?")
    break_choice = break_choice.lower()
    if break_choice == "yes":
        print("The door easily gives and falls to the side. {name} steps through, making his way into the next room.".format(name=character_select.name))
        ...
        if investigate_choice == "investigate":
            print("Moving toward the case, he sees there is a lock, but it appears to be broken.")
            character_select.spring_trap()
            print("The case was trapped all along! Proceed with extra caution moving forward!")
            input("Ready to move onward?")
            print("{name} moves forward, to the other side of the room.".format(name=character_select.name))
        if investigate_choice == "move on":
            print("Avoiding the questionable case, {name} sneaks toward the other side of the room.".format(name=character_select.name))
    if break_choice == "no":
        print("Deciding against the door, he moves back to look for the other exit.")
        room_choice = "crevice"
if room_choice == "crevice":
    print("{name} makes his way over to the crevice in the wall. \nIt's easy enough to climb through to the other side.".format(name=character_select.name))
    ...
    print(skel)
    while skel.hp > 0:
        fight_choice = input("Choose your action: \n Attack \t Use Potion \t Defend \t Run")
        ...
        kill_count += 1
        print("{name} has defeated {killcount} foes.".format(name=character_select.name, killcount = kill_count))
        print("Defeating the foe, {name} makes his way to the other side of the room.".format(name=character_select.name))
...
while door_choice != "pick lock" and door_choice !="break chains":
    door_choice = input("You have not chosen 'Pick Lock' or 'Break Chains'. Please choose your fate.")
    door_choice = door_choice.lower()
if door_choice == "pick lock":
    print("Finding a small shiv on the floor, he uses the scrap to move the locking mechanism around until a quiet 'click' loosens the locked chains.\nDeftly, {name} steps through the entrance.".format(name=character_select.name))
if door_choice == "break chains":
    print("Using naught but sheer strength and will, he charges the wooden door. \nHis shoulder finds contact with wood and the chained door bursts into splinters and scrap.")
    ...
    print("{name} is through to the next room - but the crash has attracted some unwarranted attention! \nA bandit, waiting around the corner, springs forth to attack!".format(name=character_select.name))
    print(band)
    while band.hp > 0:
        fight_choice = input("Choose your action: \n Attack \t Use Potion \t Defend \t Run")
        fight_choice = fight_choice.lower()
        ...
        if equip_sword_choice == "yes":
            character_select.has_sword = True
            print("The sword feels heavy in your hand, and gleams in the little light there is in the room.")
        if equip_sword_choice == "no":
            print("Not trusting the blade, it is left lying beside its stricken owner.")
...
while continue_choice != "yes":
    input("There is no rest in this place. Move on?")
    continue_choice = "yes"
if continue_choice == "yes":
    print("{name} makes his way through the room, climbing over heaps of crates and abandoned boxes.".format(name=character_select.name)) 
    trap_chance()
...
while door_choice != "door" and door_choice != "arch":
    door_choice = input("You have chosen unwisely. Select 'door' or 'arch'.")
    door_choice = door_choice.lower()
if door_choice == "door":
    booty_choice = input("Peaking inside the door, he clearly sees a large chest sitting in the corner of the room.\nPlunder the newly found booty?")
    booty_choice = booty_choice.lower()
    while booty_choice != "yes" and booty_choice != "no":
        booty_choice = input("Are you looting the chest?")
        booty_choice = booty_choice.lower()
    if booty_choice == "yes":
    ...
    if booty_choice == "no":
        print("Not trusting the gilded chest, he leaves the room behind, still searching for a way through.")
if door_choice == "arch":
    print("The arch, overgrown with foliage, promises some sort of pathway to the outside world. \nHowever, after moving into the room, {name} can only see shafts of moonlight splaying through the open ceiling, some 30 feet above.".format(name=character_select.name))
    ...
    while black.hp > 0:
        ...
        if don_armor_choice == "yes":
            character_select.has_armor = True
        equip_sword_choice = input("Take sword?")
        equip_sword_choice = equip_sword_choice.lower()
        if equip_sword_choice == "yes":
            character_select.has_sword = True
    print("Slaying another foe, {name} grows a taste for the action. He leaves the room behind, in search of more.".format(name=character_select.name))

##############################################################################################################
### third branch

print("From one dark, damp room into another, {name} takes his time sneaking down a long corridor. \nWhat little light there is comes from failing distant sconces.".format(name=character_select.name))
...
while path_choice != "left" and path_choice != "right" and path_choice != "forward":
    path_choice = input("Choose again: left, right, or forward.")
    path_choice = path_choice.lower()
if path_choice == "left":
    print("This door glows with some power unknown.")
    print("There appears to be no handle. Throwing a shoulder into the door does nothing.")
    door_choice = input("{name} feels the need to place his hand against the engraving. Touch it? or Walk away?")
    door_choice = door_choice.lower()
    while door_choice != "touch it" and door_choice != "walk away" and door_choice != "touch":
        door_choice = input("Choose to: Touch It or Walk Away.")
        door_choice = door_choice.lower()
    if door_choice == "touch it" or door_choice == "touch":
        print("Pressing a hand across the foreign words, {name} feels a warmth surrounding his body. He can't read the words, but he knows what they say. \nThis path is a test. \nIn order to pass through, the correct answer must be given, or dire consequences will fall upon the entrant.".format(name=character_select.name))
        ...
        print("I have cities, but no houses. I have mountains, but no trees. I have water, but no fish.")
        answer = input("What Am I?")
        answer = answer.lower()
        while answer != "a map":
            print("A shock of pain rips through your body, from your fingertips all the way through your toes.")
    ...
    if door_choice == "walk away":
        print("Fearing what danger the door possesses, he turns back to the long corridor.")
if path_choice == "right":
    print("The lifeless void of a door stares back as {name} studies the path to the right. \nThe right decision must be made. He reaches out to open what must surely be the exit.".format(name=character_select.name))
...
if path_choice == "forward":
    print("Not wanting to stray from the straight path, {name} ignores the two doors and heads down the long, empty corridor.".format(name=character_select.name))
...
if speed_choice == "sneak":
    ...
    if speed_choice2 == "faster":
        print("He takes the necessary risk and pushes forward with a burst of speed.")
        trap_chance
    if speed_choice2 == "continue":
        print("Almost to the other side, the faint clanging has turned to solid footsteps. They're not far behind, but the end of the hall is still a piece off.")
        ...
        if speed_choice3 == "run":
            print("With the end in sight, {name} breaks for the door.".format(name=character_select.name))
            trap_chance
        if speed_choice3 == "slow":
            print("Slow and sure wins the race, is what {name} always said. \n Unfortunately going so slowly comes with a price - an enemy Knight has rushed you!".format(name=character_select.name))
            print(black3)
            while black3.hp > 0:
                fight_choice = input("Choose your action: \n Attack \t Use Potion \t Defend \t Run")
                fight_choice = fight_choice.lower()
...
if speed_choice == "hurry":
    print("Wanting to get out of this hell hole, {name} springs forward to the other end of the hall, heedless of danger or trap!".format(name=character_select.name))
    character_select.spring_trap()
...
while door_choice != "pick the lock" and door_choice != "break the door" and door_choice != "break the door down":
    door_choice = input("Select: 'Pick the Lock' or 'Break the Door'.")
    door_choice = door_choice.lower()
if door_choice == "pick the lock":
    print("Using a makeshift pick found along the dungeon, he deftly opens the rudimentary door lock without problem.")
if door_choice == "break the door" or door_choice == "break the door down":
    print("Smashing through the wooden door, he remains unscathed save for a cut across the arm.")
    character_select.lose_health(1)
print("{name} certainly did not expect this scene on the other side of that simple door. \nAbout 20 meters away rests a large, dark wooden desk. A tiny curious creature, deep in thought and count, sits very still, besides its curling fingers that quickly count a large sum of gold coins. \nOn all sides of the petite creature, flowing down from the desk and filling the spacious room around us, lies innumerable gold pieces. \nWithout paying {name} the slightest mind, the creature snaps his grotesque fingers. \nSuddenly, the floor drops out from below.".format(name=character_select.name))
character_select.lose_health(3)
print("Forcibly brought to his knees from the drop, {name} must quickly decide his next action. He knew the small creature must've been close to the exit! But where to go? \n There are a few options to try: Climb back up to the Goblinoid creature, Sprint out of this Room and on toward the Exit, or Investigate this room and Prepare for what is coming.".format(name=character_select.name))
...
if choice_select == "climb":
    ...
    if climb_chance > 6:
        ...
        print("You step through the door into a strangely open, barren room.")
    else:
        print("Using his athletic prowess, he takes a running leap up toward the gaping ceiling. After a great first jump and a few frantic handholds, {name} slips and goes plummeting back to the earth.".format(name=character_select.name))
...
if choice_select == "sprint":
    ...
    if crevice_choice == "yes":
        ...
        if room_choice == "ladder":
            print("Making his way over to the ladder, {name} finds the rungs and hauls himself up through the hole in the ceiling into the room above.".format(name=character_select.name))
            print("The new room he finds himself in is strangely barren as he looks around.")
    else:
        print("The crevice looks too narrow and unsafe. He turns back to the open room.")
        choice_select == "investigate"
if choice_select == "investigate":
    print("Although he realizes this room must be some sort of trap, {name} believes there must be more here than meets the eye. \nBesides the trap door above and the crevice in the far wall, there are 3 pillars in the middle of the room.".format(name=character_select.name))
    ...
    while pillar_choice != "press the button" and pillar_choice != "turn the dial" and pillar_choice != "drink the vial":
        pillar_choice = input("Please choose: 'Press the Button', 'Turn the Dial', or 'Drink the Vial'.")
        pillar_choice = pillar_choice.lower()
    if pillar_choice == "press the button":
        print("Tempted by the gratification of pressing a small button, {name} reaches forward. As soon as it is depressed, hell rains down. \nMolten fire quickly engulfs his body.".format(name=character_select.name))
    ...
    if pillar_choice == "turn the dial":
        print("As he forces the dial to turn clockwise, it causes some sort of contraption to unwind - revealing a twisting staircase to another room below.")
...
print("Looking around the barren room, he sees naught but the way he came in. \nBut something calls out, there seems something peculiar about this room.")
...
while search_choice != "wall" and search_choice != "floor":
    search_choice = input("Choose 'Wall' or 'Floor'.")
    search_choice = search_choice.lower()
if search_choice == "wall":
    print("{name} begins to look for secrets by pressing his hand against the wall closest, then slowly circles the perimeter.".format(name=character_select.name))
    ...
    while step_choice != "yes" and step_choice != "no":
        step_choice = input("Choose Yes or No")
        step_choice = step_choice.lower()
    if step_choice == "yes":
        print("Starting with his hand, then arm, then shoulder - he watches his body disappear into the next room. \nDipping his head through, he sees what looks like an armory. Best to fully stock up.")
        ...
        character_select.num_potions += 2
    if step_choice == "no":
        print("Fearing the mysterious mirage, he heads back to search for another option.")
        search_choice = "floor"
if search_choice == "floor":
    print("Searching for some crack or crease in the tile, {name} crawls on his hands and knees along the floor.".format(name=character_select.name))
    ...
    print("A wall slides away, revealing an adjacent room. It appears to be an armory. Best to fully stock up.")
    print("Unfortunately, just as {name} reaches for a shiny new sword, two guards spring out of the darkness!".format(name=character_select.name))
    print("The first Guard immediately charges, blade lowered!")
    while gard.hp > 0:
        fight_choice = input("Choose your action: \n Attack \t Use Potion \t Defend \t Run")
...
while go_choice != "yes":
    print("There is no other way forward. He must face his fate!")
    print("\nHe inches forward into the great room, anxiety rippling through his body.")
if go_choice == "yes":
    print("He strides forward with confidence into the great room unknown.")
...
while run_choice != "run back" and run_choice != "walk forward" and run_choice != "yell":
    run_choice = input("Choose 'Run Back', 'Walk Forward', or 'Yell'.")
    run_choice = run_choice.lower()
    print("As the reality of the situation dawns on him, the doors slam shut tight behind.")
if run_choice == "run back":
    print("{name} turns just in time to see the doors seal. The crowd cheers and jeers once more.".format(name=character_select.name))
...
if run_choice == "walk forward":
    print("{name} walks boldly forward toward the center of the room, glancing around at the masses gathered. Many creatures hiss at his chutzpah.".format(name=character_select.name))
if run_choice == "yell":
    print("Echoing their chaos, {name} leaps forward and throws his head back in a wild war cry! The mass of Goblinoids goes wild in return.".format(name=character_select.name))
...
continue_choice = continue_choice.lower()
while continue_choice != "yes":
    print("That's too bad. The time has come for battle.")
    continue_choice == "yes"
if continue_choice == "yes":
    print("{name} grits his teeth and bares his knuckles over the hilt of his sword. Not knowing what threat lies ahead, his fear begins to grow.".format(name=character_select.name))
...
print("The army of Goblinoids is baffled. The cacophony ceases for a moment as the dragon's body falls lifeless.")
...
while cheer_choice != "cheer" and cheer_choice != "run":
    cheer_choice = input("Select 'Cheer' or 'Run'.")
    cheer_choice = cheer_choice.lower()
if cheer_choice == "cheer":
    print("{name} throws his head back and releases a guttural cry, reflecting those from the crowd. \nHe thrusts his sword high in the air and absorbs the ecstasy of the moment.".format(name=character_select.name))
    ...
    if fight_choice == "fight":
        print("{name} grips his longsword and swings it wildly about, in an attempt to free himself from the swarm of creatures.".format(name=character_select.name))
        print("Though he cuts through these small, grotesque creatures with ease, with each kill there in pours thrice as many foes.")
        print("{name} is quickly overrun and falls, gasping for air as the hundreds of hands twise and grip his life away.")
        character_select.die()
    else:
        print("{name} furiously erupts in a final attempt to flee.".format(name=character_select.name))
...
if cheer_choice == "run":
    print("Not missing a beat, {name} bolts past the dragon's crimson corpse, through the gargantuan double doors from whence the beast came.".format(name=character_select.name))
...
while which_choice != "go to hall" and which_choice != "search the room":
    which_choice = input("Select: 'Go to Hall' or 'Search the Room'.")
    which_choice = which_choice.lower()
if which_choice == "search the room":
    print("{name} glances quickly around the chamber, and finds it void of much of interest. \nA sole metal cylindrical container stands by the doors.".format(name=character_select.name))
    ...
    while final_chance_to_die != "open the container" and final_chance_to_die != "run":
        final_chance_to_die = input("Select: 'Open the Container' or 'Run'.")
        final_chance_to_die = final_chance_to_die.lower()
    if final_chance_to_die == "open the container":
        pass
    if final_chance_to_die == "run":
        print("Impending doom spurns on his decision as he turns his back to the container.")
        which_choice = "go to hall"
if which_choice == "go to hall":
    print("{name} now sprints through the doorway, toward what he realizes may be the exit, as moonlight pours through a windowed door at the wall opposite".format(name=character_select.name))
...
while black4.hp > 0:
    ...
    kill_count += 1
    print("{name} has defeated {killcount} foes.".format(name=character_select.name, killcount = kill_count))
print("The final foe falls freely forward, flinching and flailing freakily.")
